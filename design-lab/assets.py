"""Import approved artwork once; build literal source crops and print derivatives."""
...
BOXES={'wordmark':[0,0,256,145],'navigation':[256,0,1217,110],'left-collage':[0,145,603,868],'headline':[604,172,898,564],'intro-copy':[618,570,870,661],'menu-button':[624,674,854,725],'reservation-button':[624,732,854,786],'bottle':[900,110,1217,485],'tienda-note':[918,484,1147,640],'salud':[874,640,1007,731],'facade':[1007,640,1217,868],'feature-restaurant':[0,868,447,1086],'feature-menu':[447,868,761,1086],'feature-shop':[761,868,1217,1086],'olives':[269,919,438,1064],'herbs':[601,920,759,1066],'tin':[989,934,1185,1065],'footer':[0,1086,1217,1280]}
def get(name,url):
 p=INPUT/(name+'.png')
 if not p.exists():
  ...
  if local.exists():Image.open(local).save(p)
   ...
   try:
    with urlopen(Request(url,headers={'User-Agent':'Vineria approved image transfer'}),timeout=35)as r:raw=r.read(20000001)
    if len(raw)>20000000:raise ValueError('image size')
    im=Image.open(io.BytesIO(raw));im.load();im.save(p)
   except Exception as exc:raise RuntimeError('Approved asset '+name+' could not be retrieved: '+type(exc).__name__)from None
 print('Asset ready:',name,flush=True);return name,p
...
def halftone(image):
 ...
 a=np.clip((np.sqrt(soft/np.pi)-dist)*pitch+.5,0,1);a=np.maximum(a,np.clip((tone-.84)*7,0,1))
 result=Image.new('RGBA',im.size,(0,0,0,0));result.putalpha(Image.fromarray(np.uint8(a*255)));return result

def run():
 sources=json.loads((ROOT/'design-lab/public-images.json').read_text())
 sources['reference']=json.loads((ROOT/'design-lab/reference-image.json').read_text())['url']
 for filename in ['approved-dish-downloads.json','portrait-downloads.json']:
  for name,url in json.loads((ROOT/'design-lab'/filename).read_text()).items():
   if name in sources:sources[name]=url
 with ThreadPoolExecutor(max_workers=4)as pool:inputs=dict(pool.map(lambda kv:get(*kv),sources.items()))
 original=Image.open(inputs['reference']).convert('RGBA');original.save(OUT/'reference-original.png')
 reference=original if original.size==(1217,1280)else original.resize((1217,1280),Image.Resampling.LANCZOS)
 base=reference.copy();d=ImageDraw.Draw(base)
 ink=reference.crop((840,1184,950,1203));ink.save(OUT/'ink-tile.webp',lossless=True)
 for name,box in BOXES.items():
  tile=reference.crop(box)
  if name=='footer':
   patch=Image.new('RGBA',(650,85))
   for x in range(0,650,ink.width):
    for y in range(0,85,ink.height):patch.paste(ink,(x,y))
   tile.paste(patch,(290,39))
 ...
 report={'source_sha256':hashlib.sha256(inputs['reference'].read_bytes()).hexdigest(),'source_size':list(original.size),'composition_size':[1217,1280],'crops':BOXES,'text_style':'Printed lettering and artwork are literal source crops; editable copy uses system serif and sans-serif fonts','exceptions':['Real contact details replace placeholder information in the reference footer','Mobile layout rearranges extracted pieces for readability'],'graphics':[]}
 for name,code in POSTS.items():
  ...
  halftone(im).save(OUT/(name+'-copy.webp'),lossless=True)
 ...
 for name in ['owner','portrait']:
  im=Image.open(inputs[name]).convert('RGBA');a=im.getchannel('A');assert a.getextrema()[0]==0,name+' requires alpha'
