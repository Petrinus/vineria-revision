...
results=[]
def ck(name,value):
 results.append({'check':name,'pass':bool(value)});print(('PASS 'if value else 'FAIL ')+name,flush=True)
...
with sync_playwright() as p:
 ...
 for idea in manifest['ideas']:
  ...
  row.locator('[data-open="pair"]').click();page.frame_locator('[data-frame="restaurant"]').locator('h1').first.wait_for();page.frame_locator('[data-frame="shop"]').locator('h1').first.wait_for()
  ...
  page.locator('[data-view="all"]').click();page.frame_locator('[data-frame="management"]').locator('[data-section="overview"] h1').wait_for();ck(idea['id']+' all three visible',page.locator('[data-pane]:visible').count()==3)
  page.screenshot(path=str(out/(idea['id']+'-three-panes.png')),full_page=True)
