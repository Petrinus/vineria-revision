...
try:
 with sync_playwright()as p:
  ...
  for model in ['v1','v2','v3','v4','v5','v6a','v6b']:
  ...
  page.goto(base+'/index.html');assert page.locator('[data-proposal]').count()==7,'Do not change the existing model selection during a photo-only update'
  browser.close()
finally:server.shutdown()
report={'shops':checks,'javascript_errors':errors,'passed':len(checks)==7 and not errors}
